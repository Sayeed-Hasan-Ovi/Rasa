...
__FLASK_BASE_URL = "http://localhost:5000"
...
def get_complaint_number(sender_id, customer_number, phone_number, customer_address, electricity_office):
    ...
    print(f"customer_number: {customer_number}, phone_number: {phone_number}, customer_address: {customer_address}, electricity_office: {electricity_office}")
    dpdc_response = None
    if customer_number is None:
        try:    
            dpdc_response = dpdc_api.register_complaint_new(phone_number, customer_address, electricity_office, "52")
        except Exception as e:
            print("Exception occurred while registering complaint without customer number:", str(e))
            print("API integration have failed, please check the logs")
            return [FollowupAction("action_reset_slots")]
    else:
        try:
            dpdc_response = dpdc_api.register_complaint(phone_number, customer_number, 52)
        except Exception as e:
            print("Exception occurred while registering complaint with customer number:", str(e))
            print("API integration have failed, please check the logs")
            return [FollowupAction("action_reset_slots")]
    print(dpdc_response)
    if dpdc_response is not None:
        ...
        print("Check API logs, tracking number not found")
        return [FollowupAction("action_reset_slots")]
    ...
    def run(self,
        ...
        if requested_slot == "customer_number":
            try:
                ...
                print("Exception occurred while validating customer number for prepaid information:", str(e))
                print("API integration have failed, please check the logs")
                return [FollowupAction("action_reset_slots")]
            print(dpdc_response)
            ...
            try:
                ...
                print("Exception occurred while validating bill number for billing information:", str(e))
                print("API integration have failed, please check the logs")
                return [FollowupAction("action_reset_slots")]
            print(dpdc_response)
            ...
            repeat_counter = tracker.get_slot("repeat_counter") or 0
            dpdc_response = None
            try:
                dpdc_response = dpdc_api.online_new_connection_information(last_user_input)
            except Exception as e:
                print("Exception occurred while validating tracking number for online new connection information:", str(e))
                print("API integration have failed, please check the logs")
                return [FollowupAction("action_reset_slots")]
            print(dpdc_response)
            if dpdc_response is None or dpdc_response['HAS_ERROR'] == "Y":
                ...
                if repeat_counter >= 3:
                    ...
                    print("Incorrect tracking number provided. Giving all options.")
                    dispatcher.utter_message(json_message={
                    	"text": "",
                    	"init": "",
                		})
                ...
                print("Incorrect tracking number provided. Incrementing repeat counter.")
                print("Repeat counter:", repeat_counter)
                print("------------------------------")
                return [SlotSet("repeat_counter", repeat_counter), SlotSet(requested_slot, None)]
        ...
        # TRY REMOVING THIS
            ...
            print("No last loop found, resetting everything - From action_handle_incorrect_intent")
        print("Last intent:", last_intent)
        print("API integration might have failed, please check the logs")
        return [FollowupAction("action_reset_slots")]
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: DomainDict) -> Coroutine[Any, Any, List[Dict[str, Any]]]:
        ...
        if last_action == "utter_is_customer_number_known":
            if last_intent == "affirm":
                ...
                if last_non_affirm_deny_intent == "no_electricity": 
                    return [FollowupAction("no_electricity_form"), SlotSet("last_loop", "no_electricity_form")]
                elif last_non_affirm_deny_intent == "extra_bill":
                    return [FollowupAction("extra_bill_form"), SlotSet("last_loop", "extra_bill_form")]
            else:
            ...
            if last_intent == "affirm":
                return [
                    ...
                    SlotSet("electricity_office", None),
        ...
        # there is a loop going on
        ...
        elif last_non_affirm_deny_intent == "extra_bill" and last_action == "action_pre_loop_steps":
            print("Starting extra bill form from action handle affirm deny")
            print("Where slots - customer number:", tracker.get_slot("customer_number"), "phone number:", tracker.get_slot("phone_number"))
            if last_intent == "deny":
                return [
                    SlotSet("requested_slot", "customer_number"),
                    SlotSet("customer_number", None),
                    SlotSet('last_loop', 'extra_bill_form'), 
                    ActiveLoop("extra_bill_form"),
                ]
            elif last_intent == "affirm":
                return [SlotSet('last_loop', 'extra_bill_form'), ActiveLoop("extra_bill_form")]
        elif last_non_affirm_deny_intent == "extra_bill":
            if last_intent == "affirm":
                return [SlotSet('last_loop', 'extra_bill_form'), ActiveLoop("extra_bill_form")]
            else:
            ...
            if last_intent == "affirm":
                ...
                dispatcher.utter_message(response="utter_thankyou_message")
                return [FollowupAction("action_reset_slots")]
        if last_non_affirm_deny_intent == "no_electricity" and last_action == "action_pre_loop_steps":
            print("Starting no electricity form from action handle affirm deny")
            print("Where slots - customer number:", tracker.get_slot("customer_number"), "phone number:", tracker.get_slot("phone_number"))
            if last_intent == "deny":
                return [
                    SlotSet("requested_slot", "customer_number"),
                ...
                return [SlotSet('last_loop', 'no_electricity_form'), ActiveLoop("no_electricity_form")]
        print("No active loop found, resetting everything - From action_handle_affirm_deny")
        print("Last intent:", last_intent)
        dispatcher.utter_message(response="utter_thankyou_message")
        if last_intent == "affirm":
            dispatcher.utter_message(json_message={
...
class ActionLoopCompletion(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: DomainDict) -> Coroutine[Any, Any, List[Dict[Text, Any]]]:
        ...
        if last_loop == "no_electricity_form":
            ...
            if customer_number is not None and phone_number is not None:
                ...
                try:
                    ...
                    print("Exception occurred while fetching customer information:", str(e))
                    print("API integration have failed, please check the logs")
                    return [FollowupAction("action_reset_slots")]
        ...
        #     return [FollowupAction("action_reset_slots")]
            ...
            return [FollowupAction("action_reset_slots")]
        print("No last loop found, resetting everything")
        print("Last intent:", last_intent)
        print("API integration might have failed, please check the logs")
        return [FollowupAction("action_reset_slots")]
...
class ActionPreLoopSteps(Action):
    ...
    def run(self,
        ...
        if last_intent == "no_electricity" or last_intent == "extra_bill":
